...
class LoadSVD(gdb.Command):
    ...
    def __init__(self):
        self.vendors = {}
        try:
            vendor_names = importlib_resources.files("cmsis_svd.data").iterdir()
            for vendor in vendor_names:
                fnames = importlib_resources.files("cmsis_svd.data") / vendor.name
                self.vendors[vendor.name] = [
    ...
    def invoke(args, from_tty):
        ...
        if argc == 1:
            ...
            f = (
                importlib_resources.resource_filename("cmsis_svd")
                / "data"
                / args[0]
                / args[1]
            )
